...
exposedTimeStep = function(HPop, exposerate_df, current_day, exposed_pop_inf_prop, relative_trans_df){
  ...
  for (i in 1:length(HPop$nInf)){
    relative_expose = subset(relative_trans_df, to == HPop$names[i])$frac_move
    
    infectious_pop = HPop$nInf[i] + exposed_pop_inf_prop * HPop$nExp[i]
    infectious_pop = round(infectious_pop)
    exposerate_adj = exposerate*.5+exposerate*.5*relative_expose
    #HPop$nExposedToday[i]= sum(rbinom(infectious_pop,1,exposerate)) * (1 - ( (HPop$nInf[i] + HPop$nExp[i]) / HPop$nTotal[i]))
    HPop$nExposedToday[i]= sum(rpois(infectious_pop,exposerate_adj)) * (1 - min(1, ( (HPop$nInf[i] + HPop$nExp[i] + HPop$nRec[i]) / HPop$nTotal[i]) ))
...
###### Master function  ####
runSim = function(HPop,pat_info,control_info,mobmat,day_list,recrate_values,exposerate_df,exposepd,exposed_pop_inf_prop = 0,TSinday = 1,prob_move_per_TS=0, relative_trans_df) {
  ...
  for (current_day in 1:length(day_list)){
    ...
    HPop = exposedTimeStep(HPop,exposerate_df, day_list[current_day], exposed_pop_inf_prop, relative_trans_df)
